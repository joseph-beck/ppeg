...
import { parserInputMock } from '@/mock/parser-input-mock'
...
const useParserMutation = () => {
  ...
  return useMutation<Output, unknown, Parse>({
    ...
    mutationFn: async (parse) => {
      ...
      return axios.post('http://127.0.0.1:8080/v1/parse', JSON.parse(parserInputMock) as Parse).then((res) => res.data)
    },
    onSuccess: (data) => {
      void data
    },
